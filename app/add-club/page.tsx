...
export default function CreateClubForm() {
  ...
  const handleTitleBlur = () => {
    ...
    if (!currentSlug && clubTitle) {
      setValue("slug", generateSlug(clubTitle));
    }
  };
  ...
  const onSubmit = async (data: ClubForm) => {
    ...
    if (!data.slug) {
      data.slug = generateSlug(data.clubTitle);
    }
    try {
  ...
  const steps = ["Basic Info", "Contact", "Media", "Details"];
  const inputClass =
    "w-full h-12 px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none";
